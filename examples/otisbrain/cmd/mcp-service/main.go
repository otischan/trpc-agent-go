...
func main() {
	...
	go func() {
		<-sigChan
		fmt.Println("\nReceived shutdown signal, stopping MCP service...")
		cancel()
	}()

	// Initialize MCP toolsets based on the list of servers in config
	var mcpToolSets []*mcp.ToolSet
	...
	for _, server := range cfg.MCP.Servers {
		if server.Enabled {
			...
			if err := mcpToolSet.Init(ctx); err != nil {
				consoleLogger.Printf("Warning: Failed to initialize MCP toolset '%s': %v", server.Name, err)
			} else {
				consoleLogger.Printf("MCP Toolset '%s' initialized successfully with transport: %s", server.Name, server.Transport)
				mcpToolSets = append(mcpToolSets, mcpToolSet)
			}
		}
	}

	consoleLogger.Info("MCP service started successfully")
	
	// Keep the service running
	<-ctx.Done()
	
	// Close all MCP toolsets when shutting down
	for _, toolSet := range mcpToolSets {
		if toolSet != nil {
			toolSet.Close()
		}
	}
	
	consoleLogger.Info("MCP service stopped gracefully")
}